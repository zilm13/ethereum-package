...
def get_config(
    ...
    cmd = [
        ...
        "--rpc",
        "--rpcAddr={0}".format("::" if participant.el_ipv6_enabled else "0.0.0.0"),
        "--rpcPort={0}".format(RPC_PORT_NUM),
        "--rpcCors=*",
        "--rpcEngine",
        "--rpcEngineAddr={0}".format(
            "::" if participant.el_ipv6_enabled else "0.0.0.0"
        ),
        "--rpcEnginePort={0}".format(ENGINE_RPC_PORT_NUM),
        "--ws",
        "--wsAddr={0}".format("::" if participant.el_ipv6_enabled else "0.0.0.0"),
        "--wsPort={0}".format(WS_PORT_NUM),
        "--wsEnginePort={0}".format(WS_PORT_ENGINE_NUM),
        "--wsEngineAddr={0}".format("::" if participant.el_ipv6_enabled else "0.0.0.0"),
